...
def process_data(df):
    df.drop(columns=['Transaction Type', 'Reference Number', 'Customer Vendor #', 'Inventory Balance', 'Transaction Date.1', 'Reference Number.1', 'Created By', 'Created Date',
            'Inventory Quantity', 'Transaction Type.1', 'Created Time', 'Last Changed By', 'Last Changed Date', 'Receiving PO Number', 'Last Changed Time'], inplace=True)
    df.rename(columns={'Customer/Vendor Name': 'Customer',
              'Non-Inventory Quantity': 'Qty', 'Transaction Date': 'Date'}, inplace=True)
...
# print(df0_customer)
df0_customer.to_csv(
    r'output/customer_by_qty/370_customers_qty.csv', index=False)
...
# print(df1_customer)
df1_customer.to_csv(
    r'output/customer_by_qty/372_customers_qty.csv', index=False)
...
# df2_customer
df2_customer.to_csv(
    r'output/customer_by_qty/373_customers_qty.csv', index=False)
...
# df3_customer
df3_customer.to_csv(
    r'output/customer_by_qty/374_customers_qty.csv', index=False)

df4_customer = df4
df4_customer = top_customers(df4)
# df4_customer
df4_customer.to_csv(
    r'output/customer_by_qty/375_customers_qty.csv', index=False)

df5_customer = df5
df5_customer = top_customers(df5)
# df5_customer
df5_customer.to_csv(
    r'output/customer_by_qty/376_customers_qty.csv', index=False)
...
plt.bar(left, Gross, tick_label=tick_label,
        width=0.8, color=['blue'], label='Gross')
plt.bar(left, Net, tick_label=tick_label,
        width=0.8, color=['green'], label='Net')
